  ...
  constructor({
    ...
    eventBusService.subscribe(
      ...
      async ({ id, fulfillment_id }) => {
        ...
        const orderData = await jitsuService.buildOrder(toBuildFrom)
        const orderEvent = {
          event: "Order Shipped",
        ...
        jitsuService.track(orderEvent.event, orderEvent)
      }
    )
    ...
    eventBusService.subscribe("claim.created", async ({ id }) => {
      ...
      for (const ci of claim.claim_items) {
        ...
        const reporting_price = await jitsuService.getReportingValue(
        ...
        await jitsuService.track(event.event, event)
      }
    })
    ...
    eventBusService.subscribe("order.canceled", async ({ id }) => {
      ...
      const date = new Date()
      const orderData = await jitsuService.buildOrder(order)
      const orderEvent = {
      ...
      if (order.cart_id) {
        try {
          ...
          if (cart.context) {
            ...
            if (jitsuService.options_ && jitsuService.options_.use_ga_id) {
