...
class OrderSubscriber {
  constructor({
    ...
    this.jitsuService_ = jitsuService
    ...
    eventBusService.subscribe(
      ...
      async ({ id, fulfillment_id }) => {
        ...
        const reporting_total = await this.jitsuService_.getReportingValue(
          currency,
          total
        )

        return await jitsuService.track("Swap Shipped", {
          event: "Swap Shipped",
    ...
    eventBusService.subscribe("swap.payment_completed", async ({ id }) => {
      ...
      return await jitsuService.track("Swap Confirmed", {
    ...
    eventBusService.subscribe("swap.created", async ({ id }) => {
      ...
      return await jitsuService.track("Swap Created", {
        event: "Swap Created",
        userId: swap.order.customer_id,
  ...
  async gatherSwapReport(id) {
    ...
    const newItems = await Promise.all(
      swap.additional_items.map(async (i) => {
        ...
        const reporting_price = await this.jitsuService_.getReportingValue(
          currency,
          price
        )
