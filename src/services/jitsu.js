...
class JitsuService extends BaseService {
	/**
   * @param {Object} options - options defined in `medusa-config.js`
   *    e.g.
   *    {
   *      api_key: Jitsu api key given in Jitsu configurator
   ...
   */
	...
	constructor({ totalsService, productService }, options) {
		...
		this.jitsu_ = jitsuClient(options.api_key)
	}

	project() {
		return this.jitsu_.set({ project : 'Medusa' })
	}
	
	track(eventName, data) {
		return this.jitsu_.track(eventName, data)
	}
