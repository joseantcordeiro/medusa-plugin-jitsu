import JitsuService from "../jitsu"
...
describe("JitsuService", () => {
  ...
  describe("buildOrder", () => {
    const jitsuService = new JitsuService(
      {
    ...
    jitsuService.getReportingValue = async (_, v) => {
      const num = v
    ...
    it("successfully builds sales order", async () => {
      ...
      const jitsuOrder = await jitsuService.buildOrder(order)

      expect(jitsuOrder).toEqual({
        checkout_id: "cart_13",
        coupon: undefined,
        currency: "DKK",
        discount: 0,
